function sysCall_init() 
    ...
    if (sim.getInt32Parameter(sim.intparam_program_version)<30004) then
        sim.displayDialog("ERROR","This version of the Hokuyo sensor is only supported from V-REP V3.0.4 and upwards.&&nMake sure to update your V-REP.",sim.dlgstyle_ok,false,nil,{0.8,0,0,0,0,0},{0.5,0,0,1,1,1})
    end
end
...
function sysCall_sensing() 
    ...
    if notFirstHere then
        ...
        sim.setFloatSignal("maxScanDistance",maxScanDistance)
        sim.setFloatSignal("scanningAngle",scanningAngle)
